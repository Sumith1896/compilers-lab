...
Code_For_Ast & Array_Access_Ast::compile()
{	
	list<Icode_Stmt *> & ic_list = *new list<Icode_Stmt *>;
	Code_For_Ast & expr_ast = access_num->compile();
	if (expr_ast.get_icode_list().empty() == false)
		ic_list.splice(ic_list.end(), expr_ast.get_icode_list());
	Register_Descriptor * expr_reg = expr_ast.get_reg();
	CHECK_INVARIANT(expr_reg, "expr register cannot be null in return statement");
	...
	Icode_Stmt* curr_stmt;	
	if(get_data_type() == int_data_type) {
		Const_Opd<int> * p4 = new Const_Opd<int>(4);
		curr_stmt = new Compute_IC_Stmt(mult, expr_opd, expr_opd, p4);
	} else {
		Const_Opd<int> * p8 = new Const_Opd<int>(8);
		curr_stmt = new Compute_IC_Stmt(mult, expr_opd, expr_opd, p8);
	}
	ic_list.push_back(curr_stmt);

	Register_Descriptor * new_reg;
	if(array_symbol_entry->get_symbol_scope() == global) { // Global
		Register_Descriptor* temp_reg = machine_desc_object.get_new_register<gp_data>();
		Register_Addr_Opd * temp_opd = new Register_Addr_Opd(temp_reg);
		Const_Opd<string> * str = new Const_Opd<string>(array_symbol_entry->get_variable_name());
		curr_stmt = new Move_IC_Stmt(la, str, temp_opd);
		ic_list.push_back(curr_stmt);
		curr_stmt = new Compute_IC_Stmt(add, temp_opd, temp_opd, expr_opd);
		ic_list.push_back(curr_stmt);

		if(get_data_type() == int_data_type) {
			new_reg = machine_desc_object.get_new_register<gp_data>();
			Register_Addr_Opd * reg_opd = new Register_Addr_Opd(new_reg);
			curr_stmt = new Parameter_Store_Stmt(load, temp_opd, reg_opd, 0);
		} else {
			new_reg = machine_desc_object.get_new_register<float_reg>();
			Register_Addr_Opd * reg_opd = new Register_Addr_Opd(new_reg);
			curr_stmt = new Parameter_Store_Stmt(load_d, temp_opd, reg_opd, 0);
		}	
		temp_reg->reset_use_for_expr_result();
		ic_list.push_back(curr_stmt);

	} else { // Local
		Const_Opd<int> * offset_opd = new Const_Opd<int>(array_symbol_entry->get_start_offset());
		Register_Addr_Opd * fp_opd = new Register_Addr_Opd(machine_desc_object.spim_register_table[fp]);
		curr_stmt = new Compute_IC_Stmt(add, expr_opd, expr_opd, offset_opd);
		ic_list.push_back(curr_stmt);
	
		curr_stmt = new Compute_IC_Stmt(add, fp_opd, fp_opd, expr_opd);
		ic_list.push_back(curr_stmt);
	
		if(get_data_type() == int_data_type) {
			new_reg = machine_desc_object.get_new_register<gp_data>();
			Register_Addr_Opd * reg_opd = new Register_Addr_Opd(new_reg);
			curr_stmt = new Parameter_Store_Stmt(load, fp_opd, reg_opd, 0);
		} else {
			new_reg = machine_desc_object.get_new_register<float_reg>();
			Register_Addr_Opd * reg_opd = new Register_Addr_Opd(new_reg);
			curr_stmt = new Parameter_Store_Stmt(load_d, fp_opd, reg_opd, 0);
		}	
		ic_list.push_back(curr_stmt);
	
		curr_stmt = new Compute_IC_Stmt(sub, fp_opd, fp_opd, expr_opd);
		ic_list.push_back(curr_stmt);
	}
...
Code_For_Ast & Array_Access_Ast::create_store_stmt(Register_Descriptor * store_register)
{
	Register_Addr_Opd * reg_opd = new Register_Addr_Opd(store_register);
	list<Icode_Stmt *> & ic_list = *new list<Icode_Stmt *>;
	Code_For_Ast & expr_ast = access_num->compile();
	...
	if(array_symbol_entry->get_symbol_scope() == global) { // Global
		...
		if(get_data_type() == int_data_type) {
			curr_stmt = new Parameter_Store_Stmt(store, reg_opd, temp_opd, 0);
		} else {
			curr_stmt = new Parameter_Store_Stmt(store_d, reg_opd, temp_opd, 0);
		}	
		temp_reg->reset_use_for_expr_result();
		ic_list.push_back(curr_stmt);
	} else {
		...
		if(get_data_type() == int_data_type) {
			curr_stmt = new Parameter_Store_Stmt(store, reg_opd, fp_opd, 0);
		} else {
			curr_stmt = new Parameter_Store_Stmt(store_d, reg_opd, fp_opd, 0);
		}	
		ic_list.push_back(curr_stmt);
