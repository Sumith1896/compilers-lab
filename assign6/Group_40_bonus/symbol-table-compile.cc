...
void Symbol_Table::assign_offsets()
{
	...
	for (i = variable_table.begin(); i != variable_table.end(); i++)
	{
		...
		if (((*i)->get_symbol_scope() == local) || ((*i)->get_symbol_scope() == global))
		{
			...
			// cout << "---------------" <<endl;
			// cout << size <<endl;
			// cout << (*i)->get_start_offset() <<endl;
			// cout << (*i)->get_end_offset() <<endl;
			// cout << "---------------" <<endl;
...
int Symbol_Table::get_size_of_value_type(Symbol_Table_Entry* dt)
{
	if(dt->get_is_array()) {
		switch(dt->get_data_type())
		{
			case int_data_type: return 4*(dt->get_array_size()); break;
			case double_data_type: return 8*(dt->get_array_size()); break;
