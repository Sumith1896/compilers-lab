...
{
	CHECK_INVARIANT(typeid(*lhs)==typeid(Name_Ast) || typeid(*lhs)==typeid(Array_Access_Ast),
...
Array_Access_Ast::Array_Access_Ast(Symbol_Table_Entry & var_entry, Ast* access_ast, int line)
{
	array_symbol_entry = &var_entry;
	access_num = access_ast;
	this->node_data_type = array_symbol_entry->get_data_type();
	this->ast_num_child = zero_arity;
	this->lineno = line;
}
...
Data_Type Array_Access_Ast::get_data_type()
{
	return this->node_data_type;
}
...
{
	return (*array_symbol_entry);
}

void Array_Access_Ast::set_data_type(Data_Type dt)
{
	this->node_data_type = dt;
}

bool Array_Access_Ast::check_ast() {
	return true;
}

void Array_Access_Ast::print(ostream & file_buffer)
{
	file_buffer << "Array_Access : ";
	file_buffer << array_symbol_entry->get_variable_name() << "[";
	access_num->print(file_buffer);
	file_buffer << "]";
}
