...
%type <symbol_entry> arr_declaration
...
procedure_declaration: // MOD
	...
	if (NOT_ONLY_PARSE)
	{
		...
		CHECK_INPUT_AND_ABORT ((program_object.proc_proc_check(proc) == false),
					"Same Procedure has already been declared before", get_line_number());
...
|
	...
	// DONE
		...
		auto vv = new vector< Symbol_Table_Entry* >;
		vv->push_back($1);
		$$ = vv;
	}
	}
...
arr_declaration:
	...
	if (NOT_ONLY_PARSE)
	{
		...
		string name = *$2;
		int numb = $4;
		Symbol_Table_Entry * decl_entry = new Symbol_Table_Entry(name, int_data_type, get_line_number());
		decl_entry->set_is_array(numb);
		$$ = decl_entry;
	}
	}
|
	FLOAT NAME '[' INTEGER_NUMBER ']'
	{
	if (NOT_ONLY_PARSE)
	{
		// DONE
		...
		string name = *$2;
		int numb = $4;
		Symbol_Table_Entry * decl_entry = new Symbol_Table_Entry(name, double_data_type, get_line_number());
		decl_entry->set_is_array(numb);
		$$ = decl_entry;
	}
	}
...
return_statement:
	...
	if (NOT_ONLY_PARSE)
	{
		...
		CHECK_INPUT(current_procedure->get_return_type() == void_data_type, 
			"Return statement doesn't match with function type", get_line_number());

		Ast* return_ast = new Return_Ast(NULL,current_procedure->get_proc_label(),get_line_number());
		$$ = return_ast;
	}
	}
|
	...
	if (NOT_ONLY_PARSE)
	{
		...
		// ($1)->check_ast();
		Ast* arith = $2;
		Ast* return_ast = new Return_Ast(arith,current_procedure->get_proc_label(),get_line_number());
		$$ = return_ast;
	}
	}
;
...
|
	arr_element ASSIGN arith_expression ';'
	{
	if (NOT_ONLY_PARSE)
	{
		//ADD CODE HERE
		CHECK_INVARIANT($1 != NULL, "arr_element cannot be NULL");
		CHECK_INVARIANT($3 != NULL, "arith_expression cannot be NULL");
		Ast* var = $1;
		Ast* exp = $3;
...
arr_element:
	...
	if (NOT_ONLY_PARSE)
	{
		...
		$$ = (Ast*) new Array_Access_Ast(*var_table_entry,$3,get_line_number());
	}
	}
;
